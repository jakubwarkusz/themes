...
- Use conventional commit messages. Stable-release entries in `CHANGELOG.md` are generated from those messages.
...
API documentation belongs in `apps/docs/content/docs`; avoid parallel prop tables in README files. Releases are tag-driven from `vMAJOR.MINOR.PATCH` tags, optionally with dot-separated prerelease identifiers, and are published only by the provenance-enabled workflow. After a successful stable publish, git-cliff regenerates `CHANGELOG.md` on the default branch. Prerelease tags are omitted from that file and remain on GitHub Releases; their commits fold into the next stable section.
