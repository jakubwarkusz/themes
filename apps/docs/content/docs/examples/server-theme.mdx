...
## Cookie storage <Breaking />

<Callout type="warn">
  **1.x** automatically read the cookie on the server via `cookies()`. **`2.0.0-beta.1`** uses a static bootstrap only. See [Upgrading from 1.x](/docs/migration#upgrading-from-1x).
</Callout>
