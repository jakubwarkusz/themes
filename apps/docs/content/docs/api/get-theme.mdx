...
## Layout - apply class directly on `<html>` <Breaking />

Use this pattern when server-rendered markup itself must depend on the theme. Required more often in **`2.0.0-beta.1`** because the provider no longer injects the cookie class for you:
