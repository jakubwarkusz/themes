...
<Callout type="warn">
  <Breaking /> In **`2.0.0`**, `ThemeProvider` no longer calls `getTheme` / `cookies()` for you. Use this helper only when server markup must know the theme. See [Upgrading from 1.x](/docs/migration#upgrading-from-1x).
</Callout>
...
<Callout type="warn">
  <Breaking /> Required more often in **`2.0.0`** because the provider no longer injects the cookie class for you. Do not put unresolved `"system"` on `<html className>`. Pass `themes`, and pass `initialTheme` when seeding the provider.
</Callout>
