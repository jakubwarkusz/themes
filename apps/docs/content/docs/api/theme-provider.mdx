...
<Callout type="warn">
  <Breaking /> In 1.x, `ThemeProvider` from `@wrksz/themes/next` was an **async** Server Component that called `cookies()` for cookie/hybrid storage. From **`2.0.0-beta.1`** it is synchronous and never reads cookies on the server. See [Upgrading from 1.x](/docs/migration#upgrading-from-1x).
</Callout>
...
| `@wrksz/themes/next/extended` | Same Next bootstrap path, plus opt-in sync and system mapping | Same-document sync or custom system theme names |
| `@wrksz/themes` | Client-only provider | Nested scopes and client-rendered React |
| `@wrksz/themes/client/extended-provider` | Client-only extended provider | ShadowRoot / client-owned `themeRoot` |
...
Heavier capabilities stay behind opt-in imports so the default provider bundle does not grow. `@wrksz/themes/next/extended` is synchronous and does not call `cookies()`, matching the default Next provider.
...
<Callout type="warn">
  <Breaking /> **1.x:** `@wrksz/themes/next` called `cookies()` and could set `initialTheme` for you. **`2.0.0-beta.1`:** zero-flash comes from the pre-paint bootstrap only; the provider stays static. Explicit SSR markup still needs [`getTheme`](/docs/api/get-theme).
</Callout>
...
<Callout type="warn">
  <Breaking /> In 1.x, hybrid storage also triggered a server `cookies()` read on the Next provider.
</Callout>
...
- **App Shell:** the bootstrap reads the cookie without making the layout request-time dynamic (1.x used server `cookies()` here)
...
<Callout type="info">
  <Breaking /> New in **`2.0.0-beta.1`** as a consequence of removing implicit `cookies()`. In 1.x the Next provider was request-time for cookie/hybrid storage. See [Upgrading from 1.x](/docs/migration#upgrading-from-1x).
</Callout>
