...
<Callout type="warn">
  <Breaking /> In 1.x, `ThemeProvider` from `@wrksz/themes/next` was an **async** Server Component that called `cookies()` for cookie/hybrid storage. From **`2.0.0`** it is synchronous and never reads cookies on the server. See [Upgrading from 1.x](/docs/migration#upgrading-from-1x).
</Callout>
...
<Callout type="warn">
  <Breaking /> **1.x:** `@wrksz/themes/next` called `cookies()` and could set `initialTheme` for you. **`2.0.0`:** zero-flash comes from the pre-paint bootstrap only; the provider stays static. Explicit SSR markup still needs [`getTheme`](/docs/api/get-theme).
</Callout>
...
<Callout type="info">
  <Breaking /> New in **`2.0.0`** as a consequence of removing implicit `cookies()`. In 1.x the Next provider was request-time for cookie/hybrid storage. See [Upgrading from 1.x](/docs/migration#upgrading-from-1x).
</Callout>
