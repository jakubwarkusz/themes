...
| `disableTransitionOnChange` | `boolean \| string` | `false` | Temporarily suppress CSS transitions when switching themes. `true` disables all transitions. Pass a CSS `transition` value (e.g. `"background-color 0s, color 0s"`) to suppress only specific properties while keeping others (transforms, opacity, etc.) intact. <Since version="0.7.2" /> |
