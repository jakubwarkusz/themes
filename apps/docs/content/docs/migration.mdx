...
- **`storage="hybrid"`** - cookie-first reads for SSR + `localStorage` mirror for cross-tab sync. <Since version="0.9.0" />
...
- **`createThemes(...)`** - typed factory for provider + hooks from one canonical theme tuple. <Since version="0.9.0" />
- **`useThemeEffect(...)`** - side effects that run on theme changes after mount. <Since version="0.9.0" />
