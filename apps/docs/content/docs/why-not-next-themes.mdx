...
`storage="cookie"` is especially useful with `@wrksz/themes/next` - the static bootstrap reads the cookie before paint without opting the layout out of Next.js App Shells. <Breaking /> In 1.x the Next provider also called `cookies()` server-side for this.
